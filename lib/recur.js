...
function isExcepted(excepted, date) {
  for (let i = 0, len = excepted.length; i < len; i++) {
    if (moment(excepted[i]).isSame(date)) {
      return true
    }
  }
  return false
...
class Recur {
  constructor(options) {
    ...
    // List of rules, all of which must match
    this.rules = options.rules || []

    // List of excepted dates. Match always fails on these dates.
    // `exceptions` is deprecated.
    const excepted = options.excepted || options.exceptions || []

    this.excepted = []
    for (let i = 0; i < excepted.length; i++) {
      this.except(excepted[i])
    }
  ...
  // Export the settings, rules, and excepted of this recurring date
  save() {
    const data = {}
    ...
    data.excepted = []
    for (let i = 0, len = this.excepted.length; i < len; i++) {
      data.excepted.push(this.excepted[i].format(ISO_FORMAT))
    }
  ...
  // Creates an exception date to prevent matches, even if rules match
    ...
    this.excepted.push(d)
    return this
  }

  // Forgets rules (by passing measure) and excepted (by passing date)
  forget(dateOrRule) {
    let i, len
    let whatMoment = moment(dateOrRule, ISO_FORMAT)

    // If valid date, try to remove it from excepted
    if (whatMoment.isValid()) {
      // change to date only for perfect comparison
      whatMoment = whatMoment.dateOnly()

      for (i = 0, len = this.excepted.length; i < len; i++) {
        if (whatMoment.isSame(this.excepted[i])) {
          this.excepted.splice(i, 1)
          return this
        }
      }
  ...
  matches(dateToMatch, ignoreStartEnd) {
    ...
    if (isExcepted(this.excepted, date)) {
      return false
    }
